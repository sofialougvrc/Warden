...
The local version runs without AWS credentials. The same codebase also deploys through SST to AWS for the serverless version.
...
- **Systems component**: Rust drift-engine source
- **Infrastructure**: SST v4 on AWS
...
This is an MVP, not a full multi-tenant SaaS product. The following pieces are intentionally left as next steps:

- compiled Rust Lambda artifact,
- provider API integrations,
- auth and multi-user permissions,
- expanded CloudWatch alarm coverage,
- upload flows for new model/prompt/corpus versions.

That scope is deliberate: the repo focuses on the core control-plane workflow while keeping local development possible without cloud credentials.
...
The project remains local-first for development, while the production stage deploys the dashboard and backend resources through SST on AWS.
