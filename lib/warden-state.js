...
const stateBucket = process.env.WARDEN_STATE_BUCKET;
const stateKey = process.env.WARDEN_STATE_KEY || "state/warden-state.json";
let s3Runtime;

export async function loadState() {
  if (!stateBucket) return readLocalState();

  try {
    const { client, GetObjectCommand } = await getS3Runtime();
    ...
    const seed = await readLocalState();
...
export async function saveState(state) {
  if (stateBucket) {
    const { client, PutObjectCommand } = await getS3Runtime();
